...
def check_submission(args):
    ...
    if (branch != "target") and (branch != "sidereal"):
    ...
    return branch, calibrate_sat, detect_star, calibrate_star

def score_submission(args, branch, csat, dstar, cstar):
    ...
    elif int((len(truth_hdul)-1)/2) == 10:
        print("Scoring 10 truth submissions.")
        subs_to_score = [s for s in submission[1:] if s['file'] in ['0001.fits', '0002.fits', '0003.fits', '0004.fits', '0005.fits', '0006.fits', '0007.fits', '0008.fits', '0009.fits', '0010.fits']]
    else: 
        print(f"Scoring {len(submission)-11} submissions.")
        subs_to_score = [s for s in submission[1:] if s['file'] not in ['0001.fits', '0002.fits', '0003.fits', '0004.fits', '0005.fits', '0006.fits', '0007.fits', '0008.fits', '0009.fits', '0010.fits']]
    ...
    for sidx, sdoc in enumerate(subs_to_score):   
        ...
        for x0, y0, ra0, dec0, ra1, dec1, x1, y1, nphot, x0_FITS, x1_FITS, y0_FITS, y1_FITS, i_mag in truth_sats:
            if csat:

                c0_obs = SkyCoord(sdict['ra0'], sdict['dec0'], unit="deg")
                c1_obs = SkyCoord(sdict['ra1'], sdict['dec1'], unit="deg")
                c0_true = SkyCoord(ra0, dec0, unit="deg")
                c1_true = SkyCoord(ra1, dec1, unit="deg")
                ...
                if min_sq_error < pos_sq_error_thresh:
                    ...
                    mag_se += np.square(sdict['mag'][idx] - i_mag)
                    n_true_found_calibrate += 1
                    sub_associated_radec[idx] = True
                n_sub_assoc_calibrate += np.sum(sub_associated_radec)
            
            sq_error = np.sum([
                np.square(sdict['x0']-x0_FITS),
                np.square(sdict['y0']-y0_FITS),
                np.square(sdict['x1']-x1_FITS),
                np.square(sdict['y1']-y1_FITS)
            ], axis=0)
            idx = np.argmin(sq_error)
            min_sq_error = sq_error[idx]
            # We need to check for 0 <-> 1 label ambiguity too.
            sq_errorT = np.sum([
                np.square(sdict['x0']-x1_FITS),
                np.square(sdict['y0']-y1_FITS),
                np.square(sdict['x1']-x0_FITS),
                np.square(sdict['y1']-y0_FITS)
            ], axis=0)
            if np.min(sq_errorT) < min_sq_error:
                idx = np.argmin(sq_errorT)
                min_sq_error = sq_errorT[idx]
    ...
    scores = pd.DataFrame({'comp_name': competitor_name, 'branch': branch, 'sub_filename': args.submission, 
    ...
    if path.exists('score_history.txt'):
        scores.to_csv('score_history.txt', mode='a', index=False, header=False)
    else:
        scores.to_csv("score_history.txt", index=False)
...
if __name__ == '__main__':
    ...
    branch, csat, dstar, cstar = check_submission(args)
    score_submission(args, branch, csat, dstar, cstar)
